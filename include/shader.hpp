...
public:
    ...
    // compile shader before use (use this to declare the variable globally and then compile after the first opengl function call)
    // ------------------------------------------------------------------------
    void compile()
    {
        //global variables
    ...
    void setIntArray(const std::string &name, int amount, int *value) const
    { 
        glUniform1iv(glGetUniformLocation(ID, name.c_str()), amount, &value[0]); 
    }
    // ------------------------------------------------------------------------
